...
def filter_ground(cloud, ground_level=-1.30):
    cloud = np.array(cloud)
    x, y, z, intense = np.split(cloud, 4 , axis = -1)
...
def filter_by_distance(cloud, distance=5):
    cloud = np.array(cloud)
    x, y, z, intense = np.split(cloud, 4, axis = -1)
    print("\n Before Distance Filtering \n")
    print(cloud)
    mask_x = ((distance >= x)) & (-1*distance <= x)
    mask_y = ((distance >= y)) & (-1*distance <= y)
    mask = ((mask_x) & (mask_y))
    cloud = cloud[mask[..., 0]]
    print("\n After Distance Filtering \n")
    print(cloud)
...
def euclidean_clustering(cloud, threshold=1.5):
    cluster_labels = np.zeros(len(cloud), dtype=int)  
    
    #increment of clusters
    curr_cluster = 1
    #for loop for itereating through the clusters
    for i in range(len(cloud)):
        if cluster_labels[i] > 0:
            continue

        #Make a queue to store the point cloud data
        queue = [cloud[i]]
        cluster_labels[i] = curr_cluster

        #while loop for when item exists inside the queue
        while len(queue) > 0:
            P = queue.pop(0)
            euclidean_distance = np.sqrt((cloud[:,0]-P[0])** 2 + (cloud[:,1]-P[1] + cloud[:,2]-P[2])**2)
            
            #create a mask for applying when the distance is valid
            mask = euclidean_distance < threshold
            
            #apply the previous mask to a valid mask created below
            v_mask = mask & (cluster_labels == 0)
            v_mask = np.logical_and (mask, cluster_labels == 0)
        
        queue.extend(cloud[v_mask])
        cluster_labels[v_mask] = curr_cluster    
